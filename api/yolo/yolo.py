...
draw_detections.restype = POINTER(IMAGE)
...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45, save=True, out_dir='pred'):
    im = load_image(image, 0, 0)
    num = c_int(0)
    pnum = pointer(num)
    predict_image(net, im)
    dets = get_network_boxes(net, im.w, im.h, 0.05, hier_thresh, pnum, 1, pnum)
    ...
    for j in range(num):
        for i in range(meta.classes):
            if dets[j].prob[i] > 0:
                b = dets[j].bbox
                res.append({'class': meta.names[i].decode(), \
                            'prob': dets[j].prob[i], \
                            'pos': (b.x, b.y, b.w, b.h) \
                            })
